...
@section('content')
    ...
    <form id="create_assignment" method="POST" action="{{ action('AssignmentController@store', $section->id) }}">
            {{ csrf_field() }}
            ...
            <div class="p-3 border-l-2 border-b-0 border-r-2">
                ...
                {{-- Begin Description Input--}}
                    ...
                    <div class="mb-2">
                        ...
                        <textarea id="description" class="w-full mt-2 rounded p-2 border text-gray-600 text-sm leading-snug {{ $errors->has('description') ? 'border-red-500' : 'border' }}" name="description" value="{{ old('description') }}" >{{ old('description') }}</textarea>
                ...
                {{-- Begin Due Date Input--}}
                    ...
                    <div class="mb-2">
                        ...
                        <input id="date_due" type="text" class="w-full mt-2 rounded h-8 text-md p-1  border text-gray-600 text-sm {{ $errors->has('date_due') ? 'border-red-500' : 'border' }}" name="date_due" value="{{ old('date_due') }}" >
                    ...
                    <div class="my-1 text-center">
                          ...
                          <a href="{{action('SectionController@show', $section)}}" class="inline-block mb-1 md:mb-0 bg-gray-400 hover:bg-red-500 text-gray-700 hover:text-white px-4 py-2 text-sm uppercase tracking-wide font-semibold rounded">Cancel</a>
